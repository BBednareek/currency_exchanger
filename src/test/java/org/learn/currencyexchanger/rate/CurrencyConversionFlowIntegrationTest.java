...
@Transactional
class CurrencyConversionFlowIntegrationTest {
    ...
    @Test
    void shouldFetchRateConvertAmountAndReuseStoredRate()
            throws Exception {
        when(referenceRateProvider.fetchLatest(PAIR))
                .thenReturn(PROVIDER_RATE);

        performConversion("100.00")
        ...
        /*
         * Druga konwersja tej samej pary powinna wykorzystać
         * kurs zapisany w PostgreSQL.
         */
        performConversion("50.00")
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.source.amount")
                        .value(50.00))
                .andExpect(jsonPath("$.target.amount")
    ...
    @Test
    void shouldConvertUsingStaleFallbackWhenProviderFails()
            throws Exception {
        ...
        referenceRateRepository.store(fallbackRate);
    ...
    @Test
    void shouldReturnServiceUnavailableWithoutUsableFallback()
            throws Exception {
        when(referenceRateProvider.fetchLatest(PAIR))
                .thenThrow(
                        new RateProviderUnavailableException()
    ...
    private ResultActions performConversion(
            String amount
    ) throws Exception {
